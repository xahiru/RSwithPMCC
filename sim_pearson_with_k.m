...
function score = sim_pearson_with_k(prefs,x1,x2,k)
...
% http://davidmlane.com/hyperstat/A51911.html
    ...
    d1 = [prefs(x1,cc);prefs(x2,cc)];
      
    d2 = find(abs(prefs(x1,cc)-prefs(x2,cc))<=k);
    
    
%     final_indexes = find(abs(prefs(x1,cc)-prefs(x2,cc))<= k);
    
     shared_items = [d1(1,d2);d1(2,d2);];
   
%     shared_items(1,:)=prefs(x1,final_indexes);
%     shared_items(2,:)=prefs(x2,final_indexes);
